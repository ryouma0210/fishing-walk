  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView ref={scrollRef} style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator>
        <ImageBackground source={japanMap} resizeMode="cover" style={styles.map}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>日本全国 Fishing</Text>
        <Text style={styles.sub}>累計 {totalSteps.toLocaleString()}歩 · 50,000歩ごとに次の都道府県を解放</Text>
        <View style={styles.progress}><View style={[styles.progressFill, { width:`${Math.min(100, (currentIndex + 1) / 47 * 100)}%` }]} /></View>
      </View>
      ...
      <Modal visible={Boolean(selected)} transparent animationType="fade" statusBarTranslucent onRequestClose={() => setSelected(null)}>
        <View style={styles.backdrop}>
          ...
          {selected && selectedState && <View style={styles.sheet}>
            <Text style={styles.sheetEyebrow}>{selectedState.unlocked ? `PREFECTURE ${FISHING_AREAS.indexOf(selected) + 1}/47` : "LOCKED AREA"}</Text>
            ...
            {!selectedState.unlocked ? (
              ...
              <Button title="この都道府県で釣る" onPress={enterArea} />
            </>}
