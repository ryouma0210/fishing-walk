...
- 飲食店・駅・公園を釣り場として利用し、300m圏内判定と歩数による場所解放
- 全画面ゲームマップ、到達範囲リング、立体釣り場ビーコン。拡大時は建物を立体表示
- 青い現在地アイコンと30秒間隔の緯度・経度更新、選択した釣り場までのアプリ内徒歩ルート・距離・所要時間
- Android Health Connect／iOS Appleヘルスケアによる今日・暦月の日別グラフ、合計・平均・最高記録
...
- 同じシリーズの帽子・服・ズボン・靴が4点揃うと衣装一式を変更可能。交換画面の人物はタップで拡大表示
...
アプリ内徒歩ルートには同じGoogle Cloudプロジェクトの `Routes API` を使用します。Routes APIを有効にし、有効期限内のキーと課金設定が必要です。
